...
async function installFirebuildLinux() : Promise<void> {
  ...
  try {
    ...
    await execBashSudo(`printf '${ppaKey}' > /etc/apt/trusted.gpg.d/firebuild-ppa.asc`);
    const series = (await getExecBashOutput(`. /etc/lsb-release ; echo $DISTRIB_CODENAME"`)).stdout;
    await execBashSudo(`printf 'deb https://ppa.launchpadcontent.net/firebuild/stable/ubuntu ${series} main universe' > /etc/apt/sources.list.d/firebuild-stable-ppa.list`);
    await execBashSudo("apt-get update");
